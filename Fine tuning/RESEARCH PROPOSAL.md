...
- **VinText & MC-OCR 2021:** Lần lượt ~43.100 ảnh chữ in crop từ ảnh tự nhiên (25.776 train, 7.220 val, 10.086 unseen test) và ~6.585 dòng chữ trên hóa đơn thực tế để củng cố năng lực xử lý chữ in biến dạng.
...
- **Bước 1: Tiền xử lý (Image Preprocessing):** Áp dụng CLAHE để cân bằng độ tương phản, sử dụng Hough Line Transform để cân chỉnh góc nghiêng. Tối ưu thuật toán đệm ảnh (Adaptive Padding) bằng giá trị trung vị và không giới hạn tỷ lệ khung hình — ảnh được co đúng tỷ lệ vào canvas 128×1536 pixel bằng phương pháp scale-to-fit, đệm phần dư bằng giá trị trung vị (median pixel padding).
...
- **Bước 5: Deterministic and Language-based Post-processing:** Sử dụng PhoBERT (Masked Language Model), Deterministic Sanitization, Geographic Entity Correction.
